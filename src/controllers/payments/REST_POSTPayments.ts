...
import { prepareAccesPayment, cancelPaymentIntent, completeAccessPayment, searchPaymentIntent, cashAccessPayment } from '../../use-cases/payments';
  ...
  try {
    ...
    if (usecase === 'cashAccessPayment') {
      if (Array.isArray(httpRequest.body)) throw new Error('This enpoint do not support arrays yet...');
        ...
        successMsg = 'Payment intent found  successfully...'
        responseData = await cashAccessPayment(data) as any;
