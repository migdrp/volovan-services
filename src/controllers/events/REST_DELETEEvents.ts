///<reference path="../../types/types.d.ts" />


import { Logger } from '../../utils';
import { deleteEvent } from '../../use-cases/events';

const log = new Logger('DELETE Events Controller')

export const DELETEEvents: Utils.RESTController = async (httpRequest, usecase) => {
  try {
    let successMsg = '';
    let responseData = [];

    if (usecase === 'deleteEvent') {
      if (Array.isArray(httpRequest.body)) throw new Error('This enpoint do not support arrays yet...');
      else {

        if (!httpRequest.body)
          throw new Error('The request body can not be empty.');

        const data: Entities.Events.EventData = httpRequest.body;

        if (httpRequest['eventData']) {
          data.modifiedBy = httpRequest['eventData']['id'];
          data.modifiedOn = Date.now();
        }

        successMsg = 'Event successfully deleted.'
        responseData = await deleteEvent(data);

      }
    }

    return {
      headers: { 'Content-Type': 'application/json' },
      statusCode: 201,
      body: {
        status: 'success',
        message: successMsg,
        data: responseData,
        count: responseData.length,
      },
    };

  } catch (err) {
    log.error(err.stack || err);
    return {
      headers: { 'Content-Type': 'application/json' },
      statusCode: 400,
      body: {
        status: 'error',
        message: err.message,
      },
    };
  }
}